...
MAIN:
{
	...
	while(<>)
	{
		...
		else
		{
			...
			$h{$key}{max_observed_heteroplasmy}=$F[11] if(!$h{$key}{max_observed_heteroplasmy} or $h{$key}{max_observed_heteroplasmy}<$F[11]);
		}
	}
	
	foreach my $key (keys %h)
	{
		print join "\t",($h{$key}{info},$h{$key}{AC_hom},$h{$key}{AC_het},$h{$key}{AC_hom}/$AN,$h{$key}{AC_het}/$AN,$AN,$h{$key}{max_observed_heteroplasmy});
		print "\n";
	}
